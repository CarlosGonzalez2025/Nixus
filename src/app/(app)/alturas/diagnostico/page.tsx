...
import {
  ...
  ArrowUp, ArrowDown, ArrowUpDown, ArrowUpToLine,
} from 'lucide-react';
...
export default function DiagnosticoAlturasListPage() {
                ...
                {paginated.map(d => {
                  ...
                  return (
                    <div
                      ...
                      onClick={() => router.push(`/alturas/diagnostico/${d.id}`)}
                    >
                      ...
                      <div className="space-y-0.5 text-xs text-muted-foreground">
                        ...
                        {d.actividadAnalizada && <p className="truncate">{d.actividadAnalizada}</p>}
                        {fecha && (
                          <div className="flex items-center gap-1.5">
...
function PageHeader({ router }: { router: ReturnType<typeof useRouter> }) {
  return (
    <div className="flex items-center justify-between gap-2 flex-wrap">
      <div>
        <button
          onClick={() => router.push('/alturas')}
          className="flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground mb-1"
        >
          <ArrowLeft className="h-3 w-3" /> Alturas
        </button>
      ...
      <div className="flex items-center gap-2">
        <Button variant="outline" size="sm"
          onClick={() => router.push('/alturas/diagnostico/importar')}>
          <Upload className="mr-2 h-4 w-4" /> Importar
        </Button>
        <Button size="sm"
